...
namespace GolfPracticeTracker.Utilities
{
    public class FileHelpers
    {
            ...
            ModelStateDictionary modelState)
        {
            ...
            // Use reflection to obtain the display name for the model property associated with this IFormFile. If a display
            // name isn't found, error messages simply won't show a display name.
            MemberInfo property =
            ...
            if (formFile.ContentType.ToLower() != "text/csv")
            {
                modelState.AddModelError(formFile.Name,
                    $"The {fieldDisplayName}file ({fileName}) must be a csv file.");
            }
            ...
            else
            {
                try
                {
                    ...
                    // The StreamReader is created to read files that are UTF-8 encoded. If uploads require some other encoding, provide the encoding in the 
                    // using statement. To change to 32-bit encoding, change new UTF8Encoding(...) to new UTF32Encoding().
                    using (
                        var reader =
                            new StreamReader(
                                ...
                                detectEncodingFromByteOrderMarks: true))
                    {
                        ...
                        // Check the content length in case the file's only content was a BOM and the content is actually empty after removing the BOM.
